...
const FoodList = ({name, calory, idx, onDelete} : {name:string, calory:number, idx:number, onDelete:(idx:number) => void}) => {
  ...
  return(
    <div className='flex w-full justify-evenly border text-base mt-3' 
      ...
      {isdelete && <button onClick={handleDelete} className="inline-block mt-1 absolute right-1"><BsTrash/></button>}
    </div>
  )
};
