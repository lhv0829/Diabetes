...
const MonthlyChart = ({sort, date, isChart} : {sort:sort, date:Date, isChart:boolean}) => {
  ...
  const getExerciseData = (date, data) => {
    const startDate = new Date(date.getFullYear(), date.getMonth(), 1);
    const endDate = new Date(date.getFullYear(), date.getMonth() + 1, 0);
  ...
  useEffect(() => {
    ...
    const tempData = {
      labels: Array.from({ length: endDate.getDate() }, (_, index) =>
        (index + 1).toString()
      ),
      datasets: [
        {
